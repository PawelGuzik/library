...
public class AuthorResource {
	...
	public Response findByFilter() {
		...
		AuthorFilter authorFilter = new AuthorFilterExtractorFromUrl(uriInfo).getFilter();
		logger.debug("Finding authors using filter: {}", authorFilter);

		PaginatedData<Author> authors = authorServices.findByFilter(authorFilter);

		logger.debug("Found {} authors", authors.getNumberOfRows());

		JsonElement jsonWithPagingAndEntries = JsonUtils.getJsonElementWithPagingAndEntries(authors,
				authorJsonConverter);
